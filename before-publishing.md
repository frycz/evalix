...
- [x] **"A scorer that raises stops the run" isn't true.** `api.py:215`: leaving the
...
- [x] **The judge score isn't range-checked.** `scorers/judge.py:74`: a reply of
...
- [x] **One non-run JSON file in `runs/` crashes every run.** `store.py:84`: a file
  containing `[1,2]` raises `AttributeError`, which isn't in the list of caught errors.
- [x] **Two runs in the same second overwrite each other.** `store.py:56`: the run filename
  only has one-second precision (2 runs produced 1 file). Add microseconds or a short
  random suffix.
- [x] **Some CLI errors print a traceback.** `cli.py:151`: a scorer file with no `score()`
...
- [x] **Error line numbers are wrong.** `cases.py:91` counts only non-empty, non-comment
...
- [x] **Add a `LICENSE` file.** The MIT licence requires shipping its text. Also set
  `license-files = ["LICENSE"]` in `pyproject.toml`.
- [x] **Keep `atm.json` and `uv.lock` out of the sdist:**
  ```toml
  [tool.hatch.build.targets.sdist]
  include = ["src", "tests", "README.md", "LICENSE"]
  ```
- [x] **Keep the version number in one place.** It's in both `pyproject.toml` and
  `__init__.py`. Use `importlib.metadata.version("evalix")` or hatch's dynamic version.
- [x] **Complete the classifiers:** Python versions (3.11-3.14), `Operating System`, and
  `Typing :: Typed`. No `License ::` classifier: it conflicts with the SPDX `license` field.

## Worth fixing (less urgent)

- [x] **Cost is under-reported.** Judge calls add tokens and cost that never reach
...
- [x] **Judge failures stop the whole run.** A temporary API error during a judge call
  becomes `ScorerError`. Consider recording it like a runner error.
- [x] **`extract_json` misses some valid JSON.** If there's text around it and a `}` inside a
  string (e.g. `Sure: {"a": "}"}`), it fails, because bracket matching doesn't account for
  strings.
- [x] **`compare` matches by substring and quietly takes the newest file,** so `v1` also
  matches `v10`. It should error when more than one file matches.
- [x] **The judge never sees `expected`,** so you can't grade against a reference answer.
- [x] **Windows:** `read_text()` has no `encoding="utf-8"`, and the output uses `✓ → ≈ ⚠`.
...
- [x] **Public API:** `compare()` returns a 5-tuple, which is awkward to change later. A
  small dataclass would be better, and now is the cheapest time to switch.
- [x] **Lint:** ruff reports 22 issues, 18 auto-fixable (unused imports, import order).
